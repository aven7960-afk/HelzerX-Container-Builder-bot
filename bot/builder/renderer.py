...
def _string_select(data: dict[str, Any], *, disabled_preview: bool = True) -> ui.Select:
    options = [discord.SelectOption(label=str(x.get("label", "Option"))[:100], value=str(x.get("value", "option"))[:100], description=str(x.get("description", ""))[:100] or None, default=bool(x.get("default", False))) for x in data.get("options", [])[:25] if isinstance(x, dict)]
...
def render_component(spec: ComponentSpec, *, interactive: bool = False) -> ui.Item | None:
    ...
    if kind == "thumbnail":
        ...
        return ui.Section(ui.TextDisplay(str(data.get("description", "Thumbnail"))[:4000]), accessory=ui.Thumbnail(url, description=str(data.get("description", ""))[:256] or None)) if url else None
    if kind == "media":
    ...
    if kind == "button": return _button(data, disabled_preview=not interactive)
    if kind == "select": return _string_select(data, disabled_preview=not interactive)
    if kind in {"user_select", "role_select", "mentionable_select", "channel_select"}: return _entity_select(kind, data, disabled_preview=not interactive)
    if kind == "section":
        ...
        else: accessory = _button(data, disabled_preview=not interactive)
        return ui.Section(ui.TextDisplay(str(data.get("content", "Section content"))[:4000]), accessory=accessory)
    if kind == "action_row":
        row = ui.ActionRow()
        for child in data.get("children", [])[:5]:
            if isinstance(child, dict) and child.get("type") == "button": row.add_item(_button(child.get("data", {}), disabled_preview=not interactive))
            elif isinstance(child, dict) and child.get("type") == "select": row.add_item(_string_select(child.get("data", {}), disabled_preview=not interactive))
        return row
    if kind == "file":
        url = str(data.get("url", "")).strip()
        return ui.File(url, spoiler=bool(data.get("spoiler", False))) if url.startswith("attachment://") else ui.TextDisplay(f"`File` preview requires an uploaded attachment: `{url or 'attachment://filename'}`")
    return ui.TextDisplay(f"> Unsupported component type: `{kind}`")


class RenderedComponentsView(ui.LayoutView):
    """Render the saved state as a Discord Components V2 message."""
