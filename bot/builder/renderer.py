...
def _select(data: dict[str, Any], *, disabled_preview: bool = True) -> ui.Select:
    options = []
    for raw in data.get("options", [])[:25]:
        if isinstance(raw, dict):
            options.append(discord.SelectOption(
                label=str(raw.get("label", "Option"))[:100],
                value=str(raw.get("value", "option"))[:100],
                description=str(raw.get("description", ""))[:100] or None,
    ...
    return ui.Select(
        ...
        options=options,
        min_values=max(0, min(int(data.get("min_values", 1)), len(options))),
        max_values=max(1, min(int(data.get("max_values", 1)), len(options))),
...
def render_component(spec: ComponentSpec) -> ui.Item | None:
    ...
    if kind == "separator":
        spacing = str(data.get("spacing", "small")).lower()
        spacing_value = getattr(discord.SeparatorSpacing, spacing, discord.SeparatorSpacing.small)
        return ui.Separator(visible=bool(data.get("visible", True)), spacing=spacing_value)
    if kind == "thumbnail":
        url = str(data.get("url", "")).strip()
        if not url:
            return None
        return ui.Thumbnail(url, description=str(data.get("description", ""))[:256] or None, spoiler=bool(data.get("spoiler", False)))
    if kind == "media":
        gallery = ui.MediaGallery()
        for raw in data.get("items", [])[:10]:
            if isinstance(raw, str) and raw.strip():
                gallery.add_item(media=raw.strip())
            elif isinstance(raw, dict) and raw.get("url"):
                gallery.add_item(media=str(raw["url"]), description=str(raw.get("description", ""))[:256] or None, spoiler=bool(raw.get("spoiler", False)))
    ...
    if kind == "section":
        ...
        accessory_kind = str(data.get("accessory", "button")).lower()
        if accessory_kind == "thumbnail" and data.get("url"):
            accessory: ui.Item = ui.Thumbnail(str(data["url"]), description=str(data.get("description", ""))[:256] or None)
        else:
            accessory = _button(data, disabled_preview=True)
        return ui.Section(ui.TextDisplay(content), accessory=accessory)
    if kind == "action_row":
        row = ui.ActionRow()
        children = data.get("children", ["button"])
        for child in children[:5]:
            if child == "button":
                row.add_item(_button(data))
            elif child == "select":
                row.add_item(_select(data))
        return row
    if kind == "file":
        url = str(data.get("url", "")).strip()
...
class RenderedComponentsView(ui.LayoutView):
    """Read-only Components V2 rendering of a saved builder state."""
