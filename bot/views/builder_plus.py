...
class ComponentEditor(ui.Modal):
    def __init__(self, owner: "BuilderPlusView", index: int):
        ...
        self.owner, self.index, self.spec = owner, index, spec
        self.fields: list[tuple[str, ui.TextInput]] = []
        data = spec.data
        if spec.type == "text":
            self._field("content", "Text content", str(data.get("content", "")), True)
        elif spec.type == "button":
            for key, label, default in (("label", "Label", "Button"), ("style", "Style: primary/secondary/success/danger/link", "primary"), ("custom_id", "Custom ID", "helzerx:button"), ("url", "URL for link style", ""), ("disabled", "Disabled: true/false", "False")):
    ...
    def _parse(self, v: dict[str, str]) -> dict:
        ...
        if t == "button":
            ...
            if style == "link" and not v["url"].strip(): raise ValueError("Link buttons require a URL.")
        ...
        if t == "select":
            ...
            for line in v["options"].splitlines()[:25]:
                ...
                if len(p) >= 2 and p[0] and p[1]: options.append({"label": p[0][:100], "value": p[1][:100], "description": p[2][:100] if len(p) == 3 else ""})
            if not options: raise ValueError("Add at least one select option.")
        ...
        if t == "media":
            ...
            if not items: raise ValueError("Add at least one media URL.")
            return {"items": items}
        if t == "thumbnail":
            if not v["url"].strip(): raise ValueError("Image URL is required.")
            return {"url": v["url"][:1000], "description": v["description"][:256]}
        if t == "separator":
            spacing = v["spacing"].lower().strip()
            if spacing not in {"small", "large"}: raise ValueError("Spacing must be small or large.")
...
class IndexSelect(ui.Select):
    def __init__(self, owner: "BuilderPlusView", action: str, placeholder: str, offset: int = 0):
        self.owner, self.action_name, self.offset = owner, action, offset
...
class BuilderPlusView(ui.LayoutView):
    ...
    def _component_summary(self) -> str:
        if not self.state.components: return f"*{emoji('WARNING')} Empty container — use **Add Component** to begin.*"
...
class MoreView(ui.LayoutView):
    def __init__(self,builder:BuilderPlusView):
        super().__init__(timeout=180); self.builder=builder; box=ui.Container(ui.TextDisplay("### Builder Tools\nManage the container, JSON and templates.")); self.select=ui.Select(placeholder="Choose an action...",options=[discord.SelectOption(label="Container Name",value="name"),discord.SelectOption(label="Accent Color",value="color"),discord.SelectOption(label="Export JSON",value="export"),discord.SelectOption(label="Import JSON",value="import"),discord.SelectOption(label="Save Template",value="save"),discord.SelectOption(label="Load Template",value="load"),discord.SelectOption(label="Delete Template",value="delete"),discord.SelectOption(label="Reset",value="reset")]); self.select.callback=self.callback; box.add_item(ui.ActionRow(self.select)); self.add_item(box)

    async def callback(self,interaction:discord.Interaction)->None:
        action=self.select.values[0]
        if action=="name":
            async def done(i,value): self.builder.state.name=value[:100]; self.builder._build(); await i.response.edit_message(view=self.builder)
            await interaction.response.send_modal(TextModal("Container Name","Name",self.builder.state.name,done))
        elif action=="color":
            async def done(i,value):
                try: color=int(value.strip().lstrip("#"),16); assert 0<=color<=0xFFFFFF
                except (ValueError,AssertionError): await i.response.send_message(f"{emoji('ERROR')} Use a valid hex color such as `5865F2`.",ephemeral=True); return
                self.builder.state.accent_color=color; self.builder._build(); await i.response.edit_message(view=self.builder)
        ...
        elif action=="export": await interaction.response.send_message(file=discord.File(io.BytesIO(self.builder.state.to_json().encode()),filename="helzerx-components-v2.json"),ephemeral=True)
        elif action=="import": await interaction.response.send_modal(JsonModal(self._import))
        elif action=="save":
            async def done(i,value):
                name=value.strip()[:50]
                if not name: await i.response.send_message(f"{emoji('ERROR')} Template name cannot be empty.",ephemeral=True); return
                self.builder.store.save(self.builder.owner_id,name,self.builder.state); await i.response.send_message(f"{emoji('SUCCESS')} Template saved: **{name}**.",ephemeral=True)
            ...
            if not names: await interaction.response.send_message(f"{emoji('WARNING')} No saved templates.",ephemeral=True); return
            ...
            async def choose(i):
                ...
                if action=="delete": self.builder.store.delete(self.builder.owner_id,name); await i.response.send_message(f"{emoji('SUCCESS')} Deleted **{name}**.",ephemeral=True); return
                payload=self.builder.store.load(self.builder.owner_id,name)
                if payload: self.builder.state=BuilderState.from_dict(self.builder.owner_id,payload); self.builder._build(); await i.response.edit_message(view=self.builder)
                else: await i.response.send_message(f"{emoji('ERROR')} Template no longer exists.",ephemeral=True)
            select.callback=choose; v=ui.LayoutView(timeout=120); v.add_item(ui.Container(ui.TextDisplay(f"### {'Load' if action=='load' else 'Delete'} Template"),ui.ActionRow(select))); await interaction.response.send_message(view=v,ephemeral=True)
        elif action=="reset": self.builder.state.clear(); self.builder._build(); await interaction.response.edit_message(view=self.builder)

    async def _import(self,interaction:discord.Interaction,raw:str)->None:
        try: self.builder.state=BuilderState.from_json(self.builder.owner_id,raw)
        except ValueError as exc: await interaction.response.send_message(f"{emoji('ERROR')} {exc}",ephemeral=True); return
        self.builder._build(); await interaction.response.edit_message(view=self.builder)
